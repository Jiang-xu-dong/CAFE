...
class LoadAnnotationsDirect:
    ...
    def __call__(self, results):
        ...
        # cates=['bakckground','person','person','person','apple','apple','banana','candy','cigarette','dog','flower','flower','gift','ground','gun','island','knife','knife','plate','rod','sunflower','target','target','vegetable','zebra']
        ...
        masks = []
        instance_ids = []
        unique_ids = np.unique(pan_mask)  # e.g. [0, 1, 2, 16]
        # id_map = {orig_id: new_id for new_id, orig_id in enumerate(unique_ids)}
        # print(id_map) 
        
        for instance_id in unique_ids:  # 0,1...n object id
            flag=0
            if instance_id==255:
            ...
            if instance_id == 0:  # no segmentation area
                ...
                # print(results['ann'])
                # print(instance_id)
                # print(id_map[instance_id])
                for item in objects_info:
                    if item['object_id']==instance_id:
                        category = item['category']
                        flag=1
                        break
                if flag==0:
                    
                    print(f"here+{instance_id}+{objects_info}+{results['ann']}")
                # print(instance_id)
                # category=cates[instance_id]
        ...
        if len(
            ...
            #gt_labels=gt_labels.reshape(-1)  # jxd加
