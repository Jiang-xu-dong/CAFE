...
class LoadOutputsFromMask2Former:
    ...
    def __init__(
        ...
        video_folder = data_root / 'nsfw' / 'frames' / video_name
        ...
        for frame_output in outputs:
            # print(type(frame_output))
            # print(f"frame_output type: {type(frame_output)}")
            # print(f"frame_output content: {frame_output}")
    ...
    def _get_binary_masks_and_query_feats(
        ...
        if len(
            ...
            return np.array([]), []
        # print(len(list(query_feat_dict.keys())))
